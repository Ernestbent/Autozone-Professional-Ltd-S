...
def execute(filters=None):
    ...
    # main query using Pypika (without GROUP_CONCAT)
    query = (
        frappe.qb.from_(so)
        .left_join(c).on(
    ...
    # get Delivery Notes using raw SQL (GROUP_CONCAT DISTINCT)
    dn_query = """
        SELECT 
    ...
    # get Sales Invoices using raw SQL (GROUP_CONCAT DISTINCT)
    si_query = """
        SELECT 
            soi.parent AS sales_order,
